...
/**
  To automatically connect to the database
    QODBC://root:@gidt.localhost
    QODBC://user:pass@localhost:3036/gidt
*/
...
void addConnectionsFromCommandline(const QStringList &args, Browser *browser)
{
    for (int i = 1; i < args.count(); ++i) {
        ...
        qDebug() << "sceme: " << url.scheme();
        qDebug() << "db name: " << url.path().mid(1);
        qDebug() << "host: " << url.host();
        qDebug() << "user: " << url.userName();
        qDebug() << "pass: " << url.password();
